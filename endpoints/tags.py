...
@router.get("/get_tags", status_code=status.HTTP_200_OK, response_model=TagRead, description="Get all tags.", tags=["Tags"])
async def get_tags():
    tag_sum = get_tag_sum(session)
    log_tag_sum(logger, tag_sum)
    return TagRead(data=get_all_tags(session))

@router.post("/increment_tag", description="Persist tag state.", status_code=status.HTTP_201_CREATED, response_model=Tag, tags=["Tags"])
